...
import { generateWord, itemToast, updateLocalStorage } from "../helpers";
...
export const WordProvider = ({ children }: any) => {
	...
	useEffect(() => {
		if (numberOfChange === 5) {
			itemToast(
				"error",
				"❌ Te quedaste sin intentos, vuelve a empezar!"
			);
			return;
		}

		if (numberOfChange > 0) {
			itemToast("info", "🏃🏽‍♂️ Daleeee no te rindas, creo que estas cerca");
		}
	}, [numberOfChange]);
	...
	const handleNumberOfChange = () => {
		if (board[numberOfChange].some((char) => char.letterGuess === "")) {
			itemToast("warning", "👮🏽‍♂️ Llena todas las casillas para continuar!");
			return;
		}
		...
		if (
			...
			itemToast("success", "✅ Que increible, lo lograste");
			return;
		}
